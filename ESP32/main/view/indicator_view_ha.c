...
static void __view_event_handler(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
    ...
    case VIEW_EVENT_HA_MESSAGE:
    {
        ...
        ESP_LOGW(TAG, "seconds: %d",message_data->seconds);
    ...
    case VIEW_EVENT_HA_MQTT_CONNECTED:
    {
        ...
        {
            struct view_data_sensor_data sensor_data;
            // lv_event_send((lv_obj_t *)all_switches[i].btn, LV_EVENT_CLICKED, NULL);
            memset(&sensor_data, 0, sizeof(sensor_data));
            sensor_data.ha_data.index = i;
            sensor_data.sensor_type = SENSOR_HA;
            strncpy(sensor_data.ha_data.value, "unknown", sizeof(sensor_data.ha_data.value) - 1);
        ...
        for (int i = 0; i<all_switches_count; i++) {
            // if (all_switches[i].value[0] == 0) {
                ...
                switch_data.index = i;
                //printf("----------------------------------------------> UNKNOWN DATA FOR SWITCH %s\n",all_switches[switch_data.index].ha_key);
    ...
    case VIEW_EVENT_WIFI_ST:
        ...
        for (int i=0; i<screen_count; i++) {
            lv_img_set_src(screens[i].ha_wifi_st, (void *)p_src);
        }
            ...
            for (int i = 0; i<MAX_STATES; i++) {
                ...
                if ((all_switches[p_data->index].states[i].state_value[0] != 0) && (all_switches[p_data->index].value[0] != 0) &&
                    (strcmp(all_switches[p_data->index].value,all_switches[p_data->index].states[i].state_value) == 0) ){
                     ...
                     /*********** STYLE ********************************************/
                     /**************************************************************/
                     lv_obj_remove_style(all_switches[p_data->index].btn, &(all_switches[p_data->index].style), 0);
                } 
            }
            //lv_event_send((lv_obj_t *)all_switches[p_data->index].btn, LV_EVENT_CLICKED, NULL);
        }
