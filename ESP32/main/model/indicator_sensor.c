...
static void __sensor_history_data_update_callback(void *arg)
{
    ...
    {
        ESP_LOGE(TAG, "The time is not right!!!");
        return;
    }
...
static void __sensor_history_data_restore(void)
{
    ...
    else
    {
        ...
        {
            ESP_LOGE(TAG, "sensor history data not find");
        }
        else
        {
            ESP_LOGE(TAG, "sensor history data read err:%d", ret);
        }
...
static int __data_parse_handle(uint8_t *p_data, ssize_t len)
{
    ...
    case PKT_TYPE_CONFIG_FILE_ACK:
    {
        // Update config status
        config_flag = CONF_STARTED;

        // read 31 bytes the string data from message (skip first byte for type)
        char *str = (char *)&p_data[1];
        // ESP_LOGI(TAG, "Received config file ack: %s", str);

        // call the config file update function
        concat_config(str, 31);
        // config_read();
        break;
    }

    case PKT_TYPE_CONFIG_FILE_COMPLETE:
    {
        config_flag = CONF_COMPLETED;

        char *str = (char *)&p_data[1];
        // ESP_LOGI(TAG, "Received config file complete: %s", str);

        concat_config(str, strlen(str));
        config_read();
        // concat_config = CONF_READY;
        break;
    }
...
#if SENSOR_COMM_DEBUG
    ESP_LOGI(TAG, "Requesting Config file from RP2040");
#endif
    __cmd_send(PKT_TYPE_CONFIG_FILE_GET, NULL, 0);
...
static void __sensor_shutdown(void)
{
    ...
    if (ret <= 0)
    {
        ESP_LOGE(TAG, "sensor shutdown fail!. %d", ret);
    }
}
...
int indicator_sensor_init(void)
{
    ...
    /* The above code is creating a new task called "esp32_rp2040_comm_task" using the xTaskCreate
    function. The task is created with a stack size of ESP32_RP2040_COMM_TASK_STACK_SIZE and a
    priority of 2. The task function is esp32_rp2040_comm_task and it is passed NULL as the task
    parameter. The last parameter is also set to NULL, indicating that no task handle is required. */
    xTaskCreate(esp32_rp2040_comm_task, "esp32_rp2040_comm_task", ESP32_RP2040_COMM_TASK_STACK_SIZE, NULL, 2, NULL);

    /* The above code is creating a new task called "sensor_history_data_updata_task" using the
    xTaskCreate function. This task has a stack size of 1024*4 bytes and a priority of 6. The task
    does not have any parameters and does not have a handle to the created task. */
